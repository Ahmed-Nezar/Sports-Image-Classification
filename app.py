...
from utils import (
    get_model_paths_by_input_size, load_model, preprocess_image,
    predict, load_metrics, get_val_accuracy, count_trainable_params
)

# Optional: list of class names for readable predictions
CLASS_NAMES = ["Football", "Basketball", "Tennis", "Running", "Swimming", "Cycling", "Volleyball"]
...
# --- Page Switcher ---
view_choice = st.sidebar.radio("🧭 Select View", ["Image Analysis", "Input Transform Showcase", "Optuna Analysis", "Classification Dashboard"])

# --- Page 1: Classification Dashboard ---
if view_choice == "Classification Dashboard":
    # --- Sidebar: Input Size Selection ---
    st.sidebar.header("Configuration")
    input_size_choice = st.sidebar.selectbox("Input Size & Source", ["128_pretrained", "224_pretrained", "224_scratch"])
    ...
    if show_plots:
        plot_mode = st.radio("Select plot view mode:", ["Static (PNG)", "Interactive (Live Chart)"], horizontal=True)
    ...
    uploaded_file = st.file_uploader("Choose a sports image", type=["jpg", "jpeg", "png"])
...
# --- Page 2: Input Transform Showcase ---
    ...
    # 📤 Upload Image
    uploaded_image = st.file_uploader("Upload a sample image to visualize transformations", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_image:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.image(train_img.permute(1, 2, 0).numpy(), caption="Train Transform", use_column_width=True)
        with col2:
            st.image(test_img.permute(1, 2, 0).numpy(), caption="Test Transform", use_column_width=True)
    ...
    with st.expander("📊 Class Distribution: Train vs Test (Side by Side)"):
        st.image("logs_analysis/distribution_of_each_class.png", caption="Class Distribution (Train and Test)")
        st.markdown("""
    ...
    with st.expander("📊 Combined Train/Test Distribution per Class"):
        st.image("logs_analysis/per_class_distribution_train_vs_test.png", caption="Combined Train vs Test Distribution")
        st.markdown("""
    ...
    with st.expander("🌈 Pixel Intensity Distribution per Class"):
        st.image("logs_analysis/Pixel_distribution_per_class.png", caption="Mean Pixel Value Distributions")
        st.markdown("""
