...
NUM_CLASSES = 7
MODEL_ROOTS = {
    "128_pretrained": "logs_128_pretrained",
    "224_pretrained": "logs_224_pretrained",
    "224_scratch": "logs_224_scratch"
}
...
def get_model_paths_by_input_size(selected_size):
    ...
    return {
        os.path.basename(d): os.path.join(ckpt_dir, d)
...
def build_model_from_name(model_name: str):
    ...
    if "resnet18" in model_name:
        ...
        model = get_scratch_model(model_name)
...
def get_scratch_model(name):
    if name.startswith("simplenet1"):
        from scratch_models import SimpleNet1
        return SimpleNet1()
    elif name.startswith("simplenet2"):
        from scratch_models import SimpleNet2
        return SimpleNet2()
    elif name.startswith("simplenet3"):
        from scratch_models import SimpleNet3
        return SimpleNet3()
    elif name.startswith("simplenet4"):
        from scratch_models import SimpleNet4
        return SimpleNet4()
